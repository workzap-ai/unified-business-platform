...
def context(permissions: frozenset[str] = ALL, products: dict[str, set[str]] | None = None) -> NavContext:
...
def test_default_main_order_is_the_product_contract():
    ...
    assert [i.definition.key for i in resolved["admin"]] == [
        "settings", "members", "roles", "branches", "departments",
        "environments", "audit", "products", "notifications",
    ]
...
def test_pi_is_a_top_level_product_with_its_own_subnavigation():
    ...
    assert [c.definition.label for c in pi.children] == [
        "PI Overview", "Inbox", "Agents", "WhatsApp", "Knowledge", "Handoffs", "Analytics",
        "Settings",
    ]
...
def test_custom_order_is_applied_and_new_modules_use_default_sort_order():
    ...
    custom = ["pi", "overview", "customers", "catalog", "inventory", "sales", "quotes",
              "orders", "billing", "finance", "hr", "reports"]
    ...
    registry.register(
        NavDefinition("subscriptions", "Subscriptions", "/subscriptions", "repeat", "main", 130,
...
def test_new_module_inserted_between_neighbours_in_custom_order():
    items = [NavDefinition(k, k, f"/{k}", "x", "main", o) for k, o in
             [("a", 10), ("b", 20), ("new", 25), ("c", 30)]]
