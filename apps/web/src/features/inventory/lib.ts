...
export function stockState(level: Pick<StockLevel, "available" | "is_low">): StockState {
...
export type AllLevels = { items: StockLevel[]; total: number; truncated: boolean };
...
export async function fetchAllLevels(): Promise<AllLevels> {
  ...
  if (pages > 1) {
    const rest = await Promise.all(
      Array.from({ length: pages - 1 }, (_, i) => inventoryService.levels({ page: i + 2, pageSize: PAGE })),
    );
...
export type ProductStock = { available: number; onHand: number; state: StockState; lines: number };

/** Sum available stock per product across variants and locations. */
export function stockByProduct(levels: StockLevel[]): Map<string, ProductStock> {
  const map = new Map<string, ProductStock>();
  for (const l of levels) {
    const current = map.get(l.product_id) ?? { available: 0, onHand: 0, state: "healthy" as StockState, lines: 0 };
  ...
  for (const value of map.values()) if (value.available <= 0) value.state = "out";
  return map;
}

/** Distinct variants matching a predicate (levels are per variant x location). */
export function distinctVariants(levels: StockLevel[], predicate: (l: StockLevel) => boolean = () => true) {
  return new Set(levels.filter(predicate).map((l) => l.variant_id)).size;
}

export type BadgeTone = "neutral" | "primary" | "success" | "warning" | "danger" | "info" | "pi" | "outline";

export const MOVEMENT_KINDS: Record<Movement["kind"], { label: string; tone: BadgeTone }> = {
...
export function signed(quantity: number) {
  return quantity > 0 ? `+${quantity.toLocaleString("en-US")}` : quantity.toLocaleString("en-US");
}
...
export function movementReferenceHref(m: Pick<Movement, "ref_type" | "ref_id">): string | null {
