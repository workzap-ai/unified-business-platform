...
const KINDS: {
...
}[] = [
  {
    value: "receipt",
    label: "Receipt",
    description: "Stock received from a supplier or production.",
  },
  {
    value: "return",
    label: "Return",
    description: "Items a customer sent back into sellable stock.",
  },
  {
    value: "adjustment",
    label: "Adjustment",
    description: "Count correction, damage or loss. Can be negative.",
  },
];
  ...
  .superRefine((value, ctx) => {
    ...
    if (quantity === 0)
      ...
      ctx.addIssue({
        ...
        path: ["quantity"],
        message:
          "Receipts and returns add stock. Enter a positive number, or use Adjustment to remove stock.",
      });
  });
...
export function AdjustStockSheet({
  ...
  const active = useMemo(
    () => locations.sorted?.filter((l) => l.status === "active") ?? [],
    [locations.sorted],
  );
  ...
  const mutation = useScopedMutation(
    (input: AdjustmentInput) => inventoryService.adjust(input),
    {
  ...
  const quantity = /^[-+]?\d+$/.test(quantityText.trim())
    ? Number(quantityText)
    : null;
  const effective =
    quantity === null
          ...
          description={
            target ? (
              <>
                {target.product_name}
                {target.variant_name &&
                target.variant_name !== target.product_name
                  ? ` · ${target.variant_name}`
                  : ""}{" "}
        ...
        >
          <DialogBody className="space-y-5">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <FormField
                ...
                help={
                  kind === "adjustment"
                    ? "Use a minus sign to remove stock, e.g. -3."
                ...
                <Input
                  ...
                  aria-describedby={
                    formState.errors.quantity
                      ? "adjust-quantity-error"
                      : "adjust-quantity-help"
                  }
                  {...register("quantity")}
                />
              </FormField>
            ...
            <FormField
              ...
              help="Recorded in the stock ledger and audit log."
            >
              <Textarea
                id="adjust-reason"
                rows={3}
            ...
            <section
              aria-label="Resulting balance"
              className="rounded-lg border border-border bg-surface-muted/60 p-3.5"
            >
              <p className="text-xs font-medium text-muted-foreground">
                Resulting balance at this location
              </p>
              {levels.isPending ? (
            ...
            {mutation.isError && (
              <InlineError
                message={errorMessage(
                  mutation.error,
                  "Couldn't record this stock change.",
                )}
              />
            )}
          </DialogBody>
          <DialogFooter>
            ...
            <Button
              ...
              disabled={mutation.isPending || !active.length}
            >
              Record change
            </Button>
          </DialogFooter>
