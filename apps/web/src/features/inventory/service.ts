...
const live: InventoryService = {
  locations: () =>
    apiRequest("GET", "/inventory/locations", z.array(locationSchema)),
  createLocation: (input) =>
    apiRequest("POST", "/inventory/locations", locationSchema, { body: input }),
  ...
  adjust: (input) =>
    apiRequest("POST", "/inventory/adjustments", movementSchema, {
      body: input,
    }),
};
...
const demo: InventoryService = {
  ...
  async createLocation(input) {
    ...
    const location: Location = {
      id: demoId("loc"),
      is_default: demoBusiness().locations.length === 0,
      status: "active",
      ...input,
    };
    ...
    const rows = levelRows()
      .filter(
        (l) =>
          ...
          (!lowOnly || l.is_low) &&
          (!search ||
            matches(l.product_name, search) ||
            matches(l.sku, search)),
      )
      .sort(
        (a, b) =>
  ...
  async movements({ page = 1, pageSize = 25, variantId }) {
    ...
    const rows = demoBusiness()
      .movements.filter((m) => !variantId || m.variant_id === variantId)
  ...
  async adjust(input) {
    ...
    if (input.kind !== "adjustment" && input.quantity < 0)
      throw new ApiError(
        ...
        "INVALID_QUANTITY",
        undefined,
        "Receipts and returns add stock",
      );
    const location =
      business.locations.find((l) => l.id === input.location_id) ??
      business.locations.find((l) => l.is_default);
    if (!location)
      throw new ApiError(
        422,
        "NO_LOCATION",
    ...
    let row = business.stock.find(
      (s) => s.variant_id === input.variant_id && s.location_id === location.id,
    );
    if (!row) {
      row = {
