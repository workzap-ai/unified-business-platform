...
function MovementsPageInner() {
  ...
  const params = {
    page,
    pageSize: PAGE_SIZE,
    variantId: state.variant || undefined,
  };
  const movements = useScopedQuery(
    ["inventory", "movements", params],
    () => inventoryService.movements(params),
    {
  ...
  const variantOptions = useMemo(() => {
    ...
    if (state.variant && !options.some((o) => o.value === state.variant))
      options.unshift({
        value: state.variant,
        label: `Variant ${shortId(state.variant)}`,
      });
    return options;
  ...
  const columns: Column<Movement>[] = [
    ...
    },
    {
      ...
      optional: true,
      cell: (m) =>
        locations.data?.find((l) => l.id === m.location_id)?.name ?? (
          <span className="text-muted-foreground">—</span>
        ),
    },
    {
      ...
      hideBelow: "sm",
      cell: (m) => (
        <Badge tone={MOVEMENT_KINDS[m.kind].tone}>
          {MOVEMENT_KINDS[m.kind].label}
        </Badge>
      ),
    },
    {
      ...
      header: "Qty",
      align: "right",
      cell: (m) => <Quantity value={m.quantity} />,
    },
    {
      key: "balance",
    ...
    },
    {
      ...
      cell: (m) => {
        ...
        if (!m.ref_type || m.ref_type === "manual")
  ...
  return (
    <PageShell>
      ...
      <Notice tone="neutral" icon={Lock} className="mb-4">
        The ledger is append-only: entries are never edited or deleted. To
        correct a mistake, record an opposite adjustment from{" "}
        <Link
      ...
      >
        ...
        {selected && (
          <Link
            href={`/catalog/products/${selected.product_id}`}
