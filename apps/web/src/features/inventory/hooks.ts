...
export const STOCK_CHANGED: unknown[][] = [
  ["inventory"],
  ["reports"],
  ["overview"],
];
...
export function useProductStock(enabled = true) {
  ...
  const map = useMemo(
    () => (levels.data ? stockByProduct(levels.data.items) : undefined),
    [levels.data],
  );
  return { ...levels, map };
}
...
export function useVariantIndex(enabled = true) {
  const levels = useAllLevels(enabled);
  const map = useMemo(
    () => (levels.data ? variantIndex(levels.data.items) : undefined),
    [levels.data],
  );
  return { ...levels, map };
}
...
export function useLocations(enabled = true) {
  ...
  const sorted = useMemo(
    () => (query.data ? sortLocations(query.data) : undefined),
    [query.data],
  );
  return { ...query, sorted };
}
