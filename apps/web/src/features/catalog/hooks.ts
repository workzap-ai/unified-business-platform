...
export function useAllProducts(enabled = true) {
  return useScopedQuery(["catalog", "products", "all"], fetchAllProducts, {
    enabled,
    staleTime: 30_000,
  });
}
...
export function useProduct(id: string) {
  return useScopedQuery(["catalog", "product", id], () =>
    catalogService.product(id),
  );
}
...
export function useDefaultCurrency() {
  ...
  const currency =
    settings.data?.default_currency ?? (settings.isError ? "USD" : undefined);
  return {
    currency,
    settled: !settings.isPending,
    lowStockThreshold: settings.data?.low_stock_threshold,
  };
}
