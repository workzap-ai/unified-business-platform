...
const schema = z.object({
  ...
  slug: z
    ...
    .regex(SLUG_PATTERN, "Use lowercase letters, numbers and single hyphens, e.g. office-chairs"),
...
export function CategoryDialog({
  ...
  const submit = handleSubmit((values) =>
    mutation.mutate(
      { name: values.name.trim(), slug: values.slug.trim(), description: values.description.trim() },
      {
        onError: (e) => {
          if (e instanceof ApiError && e.status === 409) setError("slug", { message: "Another category already uses this slug" });
        },
      },
    ),
  );
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !mutation.isPending && onOpenChange(next)}>
      <DialogContent size="sm">
        <DialogHeader title="New category" description="Group related products so customers and PI can browse them." />
        <form
          onSubmit={(e) => {
        ...
        >
          <DialogBody className="space-y-4">
            <FormField label="Name" htmlFor="category-name" required error={formState.errors.name}>
              <Input id="category-name" autoFocus placeholder="Office chairs" aria-invalid={Boolean(formState.errors.name)} {...register("name")} />
            </FormField>
            <FormField label="Slug" htmlFor="category-slug" required error={formState.errors.slug} help="Used in links and integrations. Generated from the name.">
              <Input
                id="category-slug"
                className="font-mono"
            ...
            <FormField label="Description" htmlFor="category-description" optional error={formState.errors.description}>
