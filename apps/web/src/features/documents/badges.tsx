...
export function SourceBadge({ source, className }: { source: string; className?: string }) {
  ...
  return <span className={cn("text-xs text-muted-foreground", className)}>Manual</span>;
}

export function ValidUntil({ quote, className }: { quote: Pick<Quote, "valid_until" | "status">; className?: string }) {
  ...
  return (
    <Tooltip content={state === "expired" ? "Validity date has passed" : "Expires within 3 days"}>
      <span
        className={cn(
      ...
      >
        ...
        {text}
        <span className="sr-only">{state === "expired" ? "(expired)" : "(expires soon)"}</span>
      </span>
    </Tooltip>
  );
}
