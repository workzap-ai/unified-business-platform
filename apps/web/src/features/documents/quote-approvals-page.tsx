    ...
    <PageShell>
      ...
      {query.isError ? (
        ...
        <>
          ...
          >
            {query.data.total}{" "}
            {query.data.total === 1 ? "quote needs" : "quotes need"} a decision
          </p>
...
function ApprovalCard({
  ...
  const action = useScopedMutation(
    (a: QuoteAction) => documentsService.quoteAction(quote.id, a),
    {
      invalidate: [["quotes"]],
      success: (q) =>
        q.status === "approved"
          ? `${q.number} approved`
          : `${q.number} returned to draft`,
    },
  );
  ...
  return (
    <Card className="flex h-full flex-col p-4">
      ...
      <p className="tabular mt-3 text-xl font-semibold tracking-tight">
        {formatMoney(quote.total, quote.currency)}
      </p>
      <p className="text-xs text-muted-foreground">
        Submitted {relativeTime(quote.created_at)} · valid until{" "}
        <ValidUntil quote={quote} />
      </p>

      <div className="mt-3 flex-1 rounded-lg bg-warning-soft/60 px-3 py-2.5">
        <p className="flex items-center gap-1.5 text-xs font-semibold text-warning">
          <Stamp className="size-3.5" aria-hidden="true" /> Why it needs
          approval
        </p>
        {settingsLoading ? (
          ...
          <p className="mt-1.5 text-[13px] text-foreground-secondary">
            Flagged when it was submitted. Limits may have changed since —
            review the details.
          </p>
        )}
      </div>
