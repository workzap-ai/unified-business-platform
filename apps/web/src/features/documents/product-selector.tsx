...
export function StockHint({
  ...
  const { available, loading } = useVariantStock(
    sku,
    variantId,
    ...
    >
      ...
      {short && (
        <span className="font-normal">
          {" "}
          · short by {formatNumber(quantity - available)}
        </span>
      )}
    </span>
  );
    ...
    <Dialog open={open} onOpenChange={close}>
      ...
      <DialogContent size="md" className="h-[min(560px,calc(100dvh-2rem))]">
        <DialogHeader
          ...
          description={
            product
              ? "Choose a variant. Catalog prices can't be changed here."
              : "Search your active products."
          }
        />
        {product ? (
          <VariantList
        ...
        {results.isError ? (
          ...
          <div className="px-4 py-10 text-center">
            ...
            <p className="mt-2 text-[13px] font-medium">
              {term ? `No products match “${term}”` : "No active products yet"}
            </p>
            <p className="mt-0.5 text-xs text-muted-foreground">
              Only active catalog products can be added.
            </p>
          </div>
        ) : (
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                ...
                <span className="block truncate text-xs text-muted-foreground">
                  {[
                    p.category_name,
                    `${p.variant_count} ${p.variant_count === 1 ? "variant" : "variants"}`,
                  ]
                    .filter(Boolean)
                    .join(" · ")}
                </span>
...
function VariantList({
  ...
  const detail = useScopedQuery(["catalog", "product", productId], () =>
    catalogService.product(productId),
  );
  const variants =
    detail.data?.variants.filter((v) => v.status === "active") ?? [];
  return (
    <DialogBody className="px-3 py-3">
      ...
      {detail.isError ? (
        ...
        <p className="px-3 py-8 text-center text-[13px] text-muted-foreground">
          This product has no active variants to sell.
        </p>
      ) : (
