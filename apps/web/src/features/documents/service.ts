...
const live: DocumentsService = {
  quotes: ({ page = 1, pageSize = 25, search, status, customerId }) =>
    apiRequest("GET", "/quotes", pageSchema(quoteSchema), {
      query: {
        page,
        page_size: pageSize,
        search,
  ...
  createQuote: (input) =>
    apiRequest("POST", "/quotes", quoteDetailSchema, { body: input }),
  updateQuote: (id, input) =>
    apiRequest("PATCH", `/quotes/${id}`, quoteDetailSchema, { body: input }),
  quoteAction: (id, action) =>
    apiRequest("POST", `/quotes/${id}/actions`, quoteDetailSchema, {
      body: { action },
    }),
  ...
  convertQuote: (id) =>
    apiRequest(
      "POST",
      `/quotes/${id}/order`,
      orderSchema.extend({ customer_name: z.string().nullable().catch(null) }),
    ),
  orders: ({ page = 1, pageSize = 25, search, status, customerId }) =>
    apiRequest("GET", "/orders", pageSchema(orderSchema), {
      query: {
        page,
        page_size: pageSize,
        search,
  ...
  createOrder: (input) =>
    apiRequest("POST", "/orders", orderDetailSchema, { body: input }),
  updateOrderLines: (id, lines) =>
    apiRequest("PUT", `/orders/${id}/lines`, orderDetailSchema, {
      body: { lines },
    }),
  orderAction: (id, action) =>
    apiRequest("POST", `/orders/${id}/actions`, orderDetailSchema, {
      body: { action },
    }),
};
...
function priceDocument(
  ...
  const lines = inputs.map((input, i) => {
    ...
    if (input.variant_id) {
      ...
      if (
        ...
        rule("VARIANT_UNAVAILABLE", "This item is not available");
      unit_price = entry.variant.price;
      description ||= `${entry.product.name} — ${entry.variant.name}`;
    } else if (!allowCustom)
      rule(
        "CUSTOM_PRICE_NOT_ALLOWED",
        "Custom-priced lines need a team member",
      );
    const gross = mulMoney(unit_price, input.quantity);
    if (toCents(input.discount) > toCents(gross))
      rule("INVALID_DISCOUNT", "A discount exceeds its line");
    return {
      id: demoId("line"),
      ...
      unit_price,
      discount: centsToString(toCents(input.discount)),
      line_total: sumMoney([gross, `-${input.discount}`]),
      gross,
    };
  });
  ...
  return {
    lines: lines.map(({ gross: _gross, ...l }) => l),
    subtotal,
    discount_total,
    tax_total,
...
function requiresApproval(
  ...
  return (
    source === "pi" ||
    rate > Number(business.settings.max_discount_rate) ||
    (threshold !== null && toCents(total) > toCents(threshold))
  );
}
...
function quoteView(q: QuoteDetail): QuoteDetail {
  return {
    ...
    next_actions: QUOTE_ACTIONS[q.status].filter(
      (a) => !(a === "convert_to_order" && q.order_id),
    ),
  };
}
...
function applyOrderStock(
  ...
  for (const line of order.lines) {
    ...
    let row = business.stock.find(
      (s) => s.variant_id === line.variant_id && s.location_id === main.id,
    );
    if (!row) {
      row = {
    ...
    if (next < row.reserved || next < 0)
      rule("INSUFFICIENT_STOCK", `Not enough stock for ${entry.variant.sku}`);
  }
  for (const line of order.lines) {
    if (!line.variant_id) continue;
    const entry = index.get(line.variant_id);
    if (!entry?.variant.track_inventory || !main) continue;
    const row = business.stock.find(
      (s) => s.variant_id === line.variant_id && s.location_id === main.id,
    )!;
    row.on_hand += direction * line.quantity;
    business.movements.unshift({
      id: demoId("mov"),
      variant_id: line.variant_id,
      location_id: main.id,
      quantity: direction * line.quantity,
      kind: direction < 0 ? "sale" : "return",
      reason:
        direction < 0
          ? `Order ${order.number}`
...
const demo: DocumentsService = {
    ...
    const rows = demoBusiness()
      .quotes.filter(
        (q) =>
          ...
          (!search ||
            matches(q.number, search) ||
            matches(q.customer_name, search)),
      )
      .sort((a, b) => b.created_at.localeCompare(a.created_at));
    return paginate(
      rows.map(({ lines: _l, next_actions: _n, ...q }) => q),
      page,
      pageSize,
    );
  },
  ...
  async createQuote(input) {
    ...
    const quote: QuoteDetail = {
      ...
      tax_rate: business.settings.tax_rate,
      ...
      valid_until:
        input.valid_until || dateOnly(business.settings.quote_validity_days),
      notes: input.notes,
      approved_at: null,
      sent_at: null,
      order_id: null,
  ...
  async updateQuote(id, input) {
    ...
    if (quote.status !== "draft")
      rule("QUOTE_LOCKED", "Only draft quotes can be edited");
    ...
    if (input.lines) {
      ...
      quote.requires_approval = requiresApproval(
        business,
        priced.subtotal,
        priced.discount_total,
        priced.total,
  ...
  async quoteAction(id, action) {
    await demoDelay(250);
    const quote = findQuote(id);
    if (!QUOTE_ACTIONS[quote.status].includes(action))
      rule(
        "INVALID_TRANSITION",
        `A quote cannot ${action.replace(/_/g, " ")} from ${quote.status.replace(/_/g, " ")}`,
      );
    const target = (
      {
        submit: quote.requires_approval ? "pending_approval" : "approved",
        approve: "approved",
    ...
    if (action === "send" && quote.valid_until < dateOnly(0))
      rule("QUOTE_EXPIRED", "This quote's validity date has passed");
    quote.status = target;
  ...
  async convertQuote(id) {
    ...
    if (quote.status !== "accepted")
      rule("QUOTE_NOT_ACCEPTED", "Only accepted quotes become orders");
    if (quote.lines.some((l) => Number(l.quantity) % 1 !== 0))
      rule("FRACTIONAL_QUANTITY", "Order quantities must be whole units");
    ...
    const order: OrderDetail = {
      ...
      lines: quote.lines.map((l) => ({
        ...
        sku: l.variant_id
          ? (index.get(l.variant_id)?.variant.sku ?? null)
          : null,
      })),
      next_actions: [],
      invoice_id: null,
    ...
    const rows = demoBusiness()
      .orders.filter(
        (o) =>
          ...
          (!search ||
            matches(o.number, search) ||
            matches(o.customer_name, search)),
      )
      .sort((a, b) => b.created_at.localeCompare(a.created_at));
    return paginate(
      rows.map(
  ...
  async updateOrderLines(id, lines) {
    ...
    if (order.status !== "draft")
      rule("ORDER_LOCKED", "Only draft orders can be edited");
    const index = variantIndex(business);
    const priced = priceDocument(
      business,
      lines.map((l) => ({
  ...
  async orderAction(id, action) {
    ...
    if (action === "confirm") {
      ...
      if (business.settings.auto_invoice_on_order_confirm) {
        ...
        business.invoices.unshift({
          ...
          status: "issued",
          issue_date: issued,
          due_date: dateOnly(business.settings.invoice_due_days),
    ...
    if (action === "cancel") {
      const invoice = business.invoices.find(
        (i) => i.order_id === order.id && i.status !== "void",
      );
      if (invoice && toCents(invoice.amount_paid) > BigInt(0))
        rule("INVOICE_HAS_PAYMENTS", "Paid invoices cannot be voided");
