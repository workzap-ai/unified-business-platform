...
export function LinesEditor({
  ...
  const addPick = (pick: PickedVariant) =>
    onChange([...lines, lineFromPick(pick)]);
  const addCustom = () =>
    onChange([
      ...lines,
      {
  ...
  const usedVariants = lines.flatMap((line) =>
  ...
  const actions = (
    <div className="flex flex-wrap items-center gap-2">
      <ProductSelector
        onPick={addPick}
        showStock={mode === "order"}
        excludeVariantIds={usedVariants}
      />
      {mode === "quote" && (
        <Button
          type="button"
    ...
    return (
      <div
        className={cn(
          ...
          listError ? "border-danger/50" : "border-border-strong",
        )}
      >
        <EmptyState
          compact
        ...
        {listError && (
          <p
            role="alert"
            className="pb-4 text-center text-xs font-medium text-danger"
          >
            {listError}
          </p>
        )}
              ...
              >
                <div className="col-span-2 min-w-0 md:col-span-1">
                  {custom ? (
                    <>
                      <label
                        htmlFor={`${id}-desc`}
                        className="text-2xs font-medium text-muted-foreground md:sr-only"
                      >
                        Service description
                      </label>
                      <Input
                ...
                <div className="min-w-0">
                  ...
                  <Input
                    ...
                    value={line.discount}
                    onChange={(e) =>
                      update(line.key, { discount: e.target.value })
                    }
                    onFocus={(e) => {
                ...
                <div className="flex min-w-0 flex-col justify-center md:h-8 md:items-end">
                  <span className="text-2xs font-medium text-muted-foreground md:sr-only">
                    Line total
                  </span>
