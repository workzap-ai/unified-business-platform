      ...
      {overview.isError ? (
        ...
        <>
          {data && !data.auto_reply_enabled && (
            ...
            >
              PI isn&apos;t answering customers right now. New messages wait in
              the inbox for your team.
            </Notice>
          )}
            ...
            <>
              <MetricGrid>
                <MetricCard
                  ...
                  icon={MessagesSquare}
                  loading={loading}
                  href="/pi/inbox?status=open"
                  value={formatNumber(data?.conversations_active ?? 0)}
                  detail={
                    data
                      ? `${formatNumber(data.conversations_total)} total`
                      : undefined
                  }
                />
                <MetricCard
                  ...
                  loading={loading}
                  href="/pi/inbox?status=open"
                  tone={data?.unresolved ? "warning" : "default"}
                  value={formatNumber(data?.unresolved ?? 0)}
                  detail="Waiting on a reply or handoff"
                />
                <MetricCard
                  label="Open handoffs"
                  icon={UserCheck}
                  ...
                  tone="pi"
                  value={formatNumber(data?.open_handoffs ?? 0)}
                  detail="Needs your team"
                />
                <MetricCard
                  label="Automation rate"
                  icon={Bot}
                  loading={loading}
                  value={formatPercent(data?.automation_rate ?? null)}
                  detail={
                    data
                      ? `${formatNumber(data.ai_responses)} AI replies`
                      : undefined
                  }
                />
                <MetricCard
                  ...
                  icon={Timer}
                  loading={loading}
                  value={formatMs(data?.avg_response_ms)}
                  detail="Customer message to PI reply"
                />
                <MetricCard
                  label="p95 reply time"
                  icon={Gauge}
                  loading={loading}
                  tone={
                    data && data.p95_response_ms > 8000 ? "warning" : "default"
                  }
                  value={formatMs(data?.p95_response_ms)}
                  detail="95% of replies are faster"
                />
                <MetricCard
                  label="Fallback events"
                  icon={Zap}
                  loading={loading}
                  tone={data?.fallback_count ? "warning" : "default"}
                  value={formatNumber(data?.fallback_count ?? 0)}
                  detail="Answered by a fallback provider"
                />
                <MetricCard
                  label="Tool calls"
                  icon={Wrench}
                  loading={loading}
                  tone={data?.tool_failures ? "warning" : "default"}
                  value={formatNumber(data?.tool_calls ?? 0)}
                  detail={
                    data
                      ? data.tool_failures
                        ? `${formatNumber(data.tool_failures)} failed`
                        : "No failures"
                      : undefined
                  }
                />
                    ...
                    data && (
                      <p className="text-[13px] text-muted-foreground">
                        <span className="tabular font-semibold text-foreground">
                          {formatNumber(data.messages_in)}
                        </span>{" "}
                        inbound ·{" "}
                        <span className="tabular font-semibold text-foreground">
                          {formatNumber(data.messages_out)}
                        </span>{" "}
                        outbound this period
                      </p>
                    )
                  }
                  actions={
                    <Link href="/pi/analytics" className={cardLink}>
                      Analytics
                    </Link>
                  }
                />
        ...
        <EmptyState
          ...
          action={
            can("pi.whatsapp.manage") ? (
              ...
              <p className="text-[13px] text-muted-foreground">
                Ask a workspace administrator to connect WhatsApp.
              </p>
            )
          }
          secondary={
  ...
  return (
    <Card className="flex flex-col">
      <CardHeader
        ...
        description="AI providers in fallback order, last 24 hours"
        icon={<Activity />}
      />
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <StatusBadge
                    status={p.status}
                    label={
                      p.status === "unconfigured" ? "Not configured" : undefined
                    }
                  />
                </div>
                {p.configured && (
                    ...
                    <div>
                      ...
                      <dd
                        className={cn(
                          ...
                          p.success_rate < 0.95 && "text-warning",
                        )}
                      >
                        {formatPercent(p.success_rate, 1)}
                      </dd>
                    </div>
                    <div>
...
function AgentActivityCard({
  ...
  const rows = [...(data?.agent_activity ?? [])].sort(
    (a, b) => b.runs - a.runs,
  );
  ...
  return (
    <Card>
      <CardHeader
        ...
        description="Runs per agent this period"
        actions={
          <Link href="/pi/agents" className={cardLink}>
            Agents
          </Link>
        }
      />
          ...
          <ul className="space-y-2.5">
            {rows.map((r) => (
              <li key={r.agent}>
                ...
                <Progress
                  value={(r.runs / max) * 100}
                  tone="pi"
                  className="mt-1 h-1"
                  aria-label={`${AGENT_LABELS[r.agent]} runs`}
                />
              </li>
            ))}
          </ul>
...
function WhatsAppCard({
  ...
  return (
    <Card>
      <CardHeader
        title="WhatsApp"
        description="Business number connection"
        icon={<MessageCircle />}
        actions={
          <Link href="/pi/whatsapp" className={cardLink}>
            Manage
          </Link>
        }
      />
          ...
          <dl className="divide-y divide-border text-[13px]">
            ...
            <div className="flex items-center justify-between gap-3 py-2">
              ...
              <dd className="font-medium">
                {wa.last_inbound_at
                  ? relativeTime(wa.last_inbound_at)
                  : "No messages yet"}
              </dd>
            </div>
          </dl>
  ...
  const stats = k
    ? [
        ...
        { label: "Ready documents", value: k.documents_ready },
        {
          ...
          value: k.documents_failed,
          danger: k.documents_failed > 0,
        },
  ...
  return (
    <Card>
      <CardHeader
        ...
        description="What PI can cite when answering"
        icon={<BookOpen />}
        actions={
          <Link href="/pi/knowledge" className={cardLink}>
            Knowledge
          </Link>
        }
      />
          ...
          <>
            <div className="grid grid-cols-2 gap-2">
              {stats.map((s) => (
                <div
                  key={s.label}
                  className="rounded-lg bg-surface-muted/70 px-3 py-2"
                >
            ...
            {k.documents_failed > 0 && (
              <Link
                href="/pi/knowledge"
                className="mt-2.5 inline-flex items-center gap-1 text-xs font-medium text-danger hover:underline"
              >
            ...
            {k.documents_ready === 0 && (
              <p className="mt-2.5 text-xs text-muted-foreground">
                No ready documents yet. PI will only answer from catalog and
                order data.
              </p>
            )}
          </>
...
function RecentConversations() {
  const query = useScopedQuery(piKeys.conversations({}), () =>
  ...
  return (
    <Card>
      <CardHeader
        ...
        actions={
          <Link href="/pi/inbox" className={cardLink}>
            Open inbox
          </Link>
        }
      />
      <div className="px-2 pb-2">
        {query.isError ? (
          ...
          <EmptyState
            ...
            description="Customer conversations appear here as soon as someone messages your WhatsApp number."
          />
        ) : (
          <ul>
            {rows.map((c) => (
  ...
  return (
    <Card>
      <CardHeader
        ...
        description="Conversations PI passed to your team"
        actions={
          <Link href="/pi/handoffs" className={cardLink}>
            All handoffs
          </Link>
        }
      />
      <div className="px-2 pb-2">
        {query.isError ? (
          ...
          <EmptyState
            ...
            icon={UserCheck}
            title="No handoffs"
            description="When PI needs a person, for a complaint, approval or low confidence, the conversation lands here."
          />
        ) : (
          <ul>
            {rows.map((h) => (
