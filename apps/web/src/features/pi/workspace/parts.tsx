...
export function usePiNames() {
  ...
  const agentNames = new Map(
    (agents.data ?? []).map((a) => [a.key as string, a.name]),
  );
  ...
  return (
    <span
      className={cn(
        ...
        withLabel &&
          (ai
            ? "bg-pi-soft px-1.5 py-0.5 text-pi-soft-foreground"
            : "bg-info-soft px-1.5 py-0.5"),
        className,
      )}
      title={label}
    >
...
export function TimeAgo({
  ...
  return (
    <time
      ...
      className={cn("tabular shrink-0", className)}
    >
      {shortTime(value)}
    </time>
  );
