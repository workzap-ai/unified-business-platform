...
export function CustomerEditDialog({
  ...
  const update = useScopedMutation((values: CustomerFormValues) => customersService.update(customer.id, formToInput(values)), {
    ...
    <Dialog open={open} onOpenChange={(next) => !saving && onOpenChange(next)}>
      <DialogContent size="md">
        <form onSubmit={onSubmit} noValidate className="flex min-h-0 flex-1 flex-col">
          <DialogHeader title="Edit customer" description="Update contact details, company and tags." />
          <DialogBody>
          ...
          <DialogFooter>
            ...
            <Button type="submit" loading={saving} disabled={saving || !form.formState.isDirty}>
              Save changes
            </Button>
          </DialogFooter>
        </form>
