...
export function OrdersTab({ customerId }: { customerId: string }) {
  ...
  const query = useScopedQuery(
    ["orders", params],
    () => documentsService.orders(params),
    { placeholderData: (p) => p },
  );
  const columns: Column<Order>[] = [
    ...
    },
    {
      ...
      cell: (o) =>
        o.source === "pi" ? (
          <Badge tone="pi">PI</Badge>
        ) : (
          <span className="text-muted-foreground">
            {o.source === "quote" ? "From quote" : "Manual"}
          </span>
        ),
    },
    {
...
export function QuotesTab({ customerId }: { customerId: string }) {
  ...
  const query = useScopedQuery(
    ["quotes", params],
    () => documentsService.quotes(params),
    { placeholderData: (p) => p },
  );
...
export function InvoicesTab({ customerId }: { customerId: string }) {
  ...
  const columns: Column<Invoice>[] = [
    ...
    },
    {
      key: "status",
      header: "Status",
      cell: (i) => <StatusBadge status={i.is_overdue ? "overdue" : i.status} />,
    },
    {
      key: "due",
      header: "Due",
    ...
    },
    {
      ...
      hideBelow: "sm",
      cell: (i) => (
        <span className="tabular">{formatMoney(i.total, i.currency)}</span>
      ),
    },
    {
      key: "balance",
      ...
      <DataTable
        ...
        empty={
          <EmptyState
            ...
            description="Invoices are created from confirmed orders or directly in Billing."
          />
        }
      />
      <Pagination
...
export function ConversationsTab({ customer }: { customer: CustomerDetail }) {
  const query = useScopedQuery(
    ["pi", "conversations", { search: customer.name }],
    () => piService.conversations({ search: customer.name }),
    {
      retry: (count, error) =>
        !(error instanceof ServiceNotConnectedError) && count < 2,
    },
  );
  const rows: Conversation[] | undefined = query.data?.items.filter(
      ...
      <div className="px-2 pb-2">
        {query.isPending || !rows ? (
          ...
          <EmptyState
            ...
            description="When this customer messages your WhatsApp number, PI's conversations appear here."
          />
        ) : (
              ...
              <li key={c.id}>
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="flex flex-wrap items-center gap-1.5">
                      ...
                      <StatusBadge
                        ...
                        dot={false}
                        label={
                          c.mode === "ai" ? "PI replying" : "Team replying"
                        }
                      />
                      {c.handoff_status && (
                        <StatusBadge
                          status={c.handoff_status}
                          label={`Handoff: ${c.handoff_status.replace(/_/g, " ")}`}
                        />
                      )}
                      {c.unread_count > 0 && (
                        <Badge tone="primary">{c.unread_count} unread</Badge>
                      )}
                    </span>
                    <span className="mt-1 block truncate text-[13px]">
                      {c.last_message_preview || c.summary}
                    </span>
                    {c.summary && c.last_message_preview && (
                  ...
                  <time
                    ...
                    title={formatDateTime(c.last_message_at)}
...
export function NotesTab({ customerId }: { customerId: string }) {
  const { can } = useSession();
  const notes = useScopedQuery(["customers", "notes", customerId], () =>
    customersService.notes(customerId),
  );
  ...
  const add = useScopedMutation(
    (values: NoteValues) =>
      customersService.addNote(customerId, values.body.trim()),
    {
      invalidate: [
        ["customers", "notes", customerId],
          ...
          <form onSubmit={form.handleSubmit((v) => add.mutate(v))} noValidate>
            <CardBody className="pt-4">
              <FormField
                ...
                error={error}
                help="Visible to your team only. PI doesn't share notes with customers."
              >
                <Textarea
                  id="customer-note"
                  rows={3}
                  ...
                  aria-describedby={
                    error ? "customer-note-error" : "customer-note-help"
                  }
                  onKeyDown={(e) => {
                    if (e.key === "Enter" && (e.metaKey || e.ctrlKey))
                      void form.handleSubmit((v) => add.mutate(v))();
                  }}
                  {...form.register("body")}
                />
              </FormField>
        ...
        <div className="px-4 pb-4">
          {notes.isError ? (
            ...
            <p className="py-6 text-center text-[13px] text-muted-foreground">
              No notes yet. Capture preferences and context your team should
              know.
            </p>
          ) : (
            <ul className="divide-y divide-border">
              {notes.data.items.map((note) => (
                <li key={note.id} className="flex gap-3 py-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-[13px] leading-relaxed break-words whitespace-pre-wrap">
                      {note.body}
                    </p>
                  </div>
                </li>
