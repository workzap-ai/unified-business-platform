...
import { Card, CardBody, CardHeader, Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/display";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
import { ActivityTimeline, PropertyList, RecordHeader, RelatedList } from "@/components/app/record";
...
import { ConversationsTab, InvoicesTab, NotesTab, OrdersTab, QuotesTab } from "./components/customer-tabs";

const TABS = ["overview", "activity", "orders", "quotes", "invoices", "conversations", "notes"] as const;
...
function CustomerDetailView({ id }: { id: string }) {
  const query = useScopedQuery(["customers", "detail", id], () => customersService.get(id));
  const customer = query.data;
  useBreadcrumbs(customer ? [{ label: customer.name }] : [], customer ? { href: `/customers/${id}`, kind: "Customer" } : undefined);
...
function CustomerRecord({ customer }: { customer: CustomerDetail }) {
  ...
  const tab: Tab = allowed.includes(state.tab) ? (state.tab as Tab) : "overview";
  const [editing, setEditing] = useState(false);
  const [confirmStatus, setConfirmStatus] = useState(false);
  const archived = customer.status === "archived";

  const setStatus = useScopedMutation(() => customersService.setStatus(customer.id, archived ? "active" : "archived"), {
    invalidate: [["customers"]],
    success: archived ? `${customer.name} restored` : `${customer.name} archived`,
    ...
    <>
      {customer.phone && (
        <a href={`tel:${customer.phone}`} className="inline-flex items-center gap-1.5 hover:text-foreground">
          <Phone className="size-3.5" aria-hidden="true" /> <span className="tabular">{customer.phone}</span>
        </a>
      )}
      {customer.email && (
        <a href={`mailto:${customer.email}`} className="inline-flex min-w-0 items-center gap-1.5 hover:text-foreground">
          <Mail className="size-3.5" aria-hidden="true" /> <span className="truncate">{customer.email}</span>
        </a>
      )}
      <span className="inline-flex items-center gap-1.5">
        <CalendarClock className="size-3.5" aria-hidden="true" />
        {customer.last_contacted_at ? `Last contacted ${relativeTime(customer.last_contacted_at)}` : "Not contacted yet"}
      </span>
    </>
  );
      ...
      {canWrite && (
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button size="icon-sm" variant="secondary" aria-label="More customer actions">
              <MoreHorizontal />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent>
            <DropdownMenuItem onSelect={() => setConfirmStatus(true)} destructive={!archived}>
              {archived ? <ArchiveRestore /> : <Archive />} {archived ? "Restore customer" : "Archive customer"}
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      )}
  ...
  return (
    <PageShell>
      ...
      <Tabs value={tab} onValueChange={(v) => setState({ tab: v }, { resetPage: false })}>
        <TabsList aria-label="Customer sections">
          <TabsTrigger value="overview">Overview</TabsTrigger>
          <TabsTrigger value="activity">Activity</TabsTrigger>
          {can("orders.read") && <TabsTrigger value="orders">Orders</TabsTrigger>}
          {can("quotes.read") && <TabsTrigger value="quotes">Quotes</TabsTrigger>}
          {can("billing.read") && <TabsTrigger value="invoices">Invoices</TabsTrigger>}
          {can("pi.read") && <TabsTrigger value="conversations">Conversations</TabsTrigger>}
          <TabsTrigger value="notes">Notes</TabsTrigger>
        </TabsList>
        <TabsContent value="overview">
          <OverviewTab customer={customer} onTab={(t) => setState({ tab: t }, { resetPage: false })} />
        </TabsContent>
      ...
      {canWrite && <CustomerEditDialog customer={customer} open={editing} onOpenChange={setEditing} />}
      <ConfirmDialog
        open={confirmStatus}
...
function OverviewTab({ customer, onTab }: { customer: CustomerDetail; onTab: (tab: Tab) => void }) {
  const { can } = useSession();
  ...
  return (
    <div className="space-y-4">
      <MetricGrid className="xl:grid-cols-4">
        <MetricCard label="Orders" icon={ShoppingCart} value={formatNumber(summary.order_count)} detail="Excluding cancelled" />
        <MetricCard label="Open quotes" icon={FileText} value={formatNumber(summary.open_quote_count)} />
        {summary.outstanding_balance !== null && (
          <MetricCard
            label="Outstanding balance"
            icon={Wallet}
            tone={Number(summary.outstanding_balance) > 0 ? "warning" : "default"}
            ...
            <CardBody className="pb-2">
              <PropertyList
                items={[
                  ...
                  { label: "Email", value: customer.email ? <a href={`mailto:${customer.email}`} className="text-primary hover:underline">{customer.email}</a> : null },
                  { label: "Phone", value: customer.phone ? <span className="tabular">{customer.phone}</span> : null },
                  { label: "Company", value: customer.company },
                  { label: "Tags", value: customer.tags.length ? <span className="inline-flex justify-end"><TagList tags={customer.tags} max={8} /></span> : null },
                  { label: "Source", value: <CustomerSourceBadge source={customer.source} /> },
            ...
            <Card>
              <CardHeader
                title={
                  <span className="flex items-center gap-1.5">
                    <Bot className="size-4 text-pi" aria-hidden="true" /> PI conversations
                  </span>
                }
              ...
              <CardBody>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <Button size="sm" variant="secondary" asChild>
                    <Link href={`/pi/inbox?search=${encodeURIComponent(customer.name)}`}>
                      <MessageSquare /> Open in PI inbox
                    </Link>
                  </Button>
                  <Button size="sm" variant="ghost" onClick={() => onTab("conversations")}>
                    View here
                  </Button>
                </div>
              </CardBody>
        ...
        <div className="space-y-4">
          {can("orders.read") && (
            <RelatedList
              ...
              getKey={(o) => o.id}
              empty={orders.isError ? "Orders couldn't be loaded." : "No orders yet."}
              action={orders.data && orders.data.total > 5 ? <TabLink onClick={() => onTab("orders")} /> : undefined}
              render={(o) => (
                <RelatedRow href={`/orders/${o.id}`} icon={ShoppingCart} title={o.number} meta={formatDate(o.created_at)} status={o.status} amount={formatMoney(o.total, o.currency)} />
              )}
            />
          )}
          {can("quotes.read") && (
            <RelatedList
              ...
              getKey={(q) => q.id}
              empty={quotes.isError ? "Quotes couldn't be loaded." : "No quotes yet."}
              action={quotes.data && quotes.data.total > 5 ? <TabLink onClick={() => onTab("quotes")} /> : undefined}
              render={(q) => (
                <RelatedRow href={`/quotes/${q.id}`} icon={FileText} title={q.number} meta={`Valid until ${formatDate(q.valid_until)}`} status={q.status} amount={formatMoney(q.total, q.currency)} />
              )}
            />
          )}
          {can("billing.read") && (
            <RelatedList
              ...
              getKey={(i) => i.id}
              empty={invoices.isError ? "Invoices couldn't be loaded." : "No invoices yet."}
              action={invoices.data && invoices.data.total > 5 ? <TabLink onClick={() => onTab("invoices")} /> : undefined}
              render={(i) => (
                <RelatedRow
                  href={`/billing/invoices/${i.id}`}
                  icon={Receipt}
                  title={i.number}
                  meta={i.due_date ? `Due ${formatDate(i.due_date)}` : formatDate(i.created_at)}
...
function RelatedRow({
  ...
  return (
    <Link href={href} className="flex items-center gap-3 rounded-lg px-2 py-2 hover:bg-surface-muted">
...
function ActivityTab({ customerId }: { customerId: string }) {
  const activities = useScopedQuery(["customers", "activities", customerId], () => customersService.activities(customerId));
  return (
    <Card className="max-w-3xl">
      <CardHeader title="Activity" description="Changes, notes, documents and conversations for this customer" />
      <CardBody>
        {activities.isError ? (
          <ErrorState error={activities.error} onRetry={() => void activities.refetch()} compact />
        ) : (
          <ActivityTimeline
