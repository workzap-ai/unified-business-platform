...
const BUILT_IN: Segment[] = [
  {
    key: "active",
    name: "All active",
    description: "Customers you can quote and sell to",
    icon: Users,
  ...
  },
  {
    key: "whatsapp",
    name: "WhatsApp",
    description: "Customers tagged whatsapp",
    icon: MessageCircle,
    filter: { tag: "whatsapp" },
  ...
  },
  {
    key: "vip",
    name: "VIP",
    description: "Customers tagged vip",
    icon: Star,
    filter: { tag: "vip" },
  ...
  },
  {
    key: "archived",
    name: "Archived",
    description: "Kept on record, hidden from pickers",
    icon: Archive,
...
function Segments() {
  const sample = useScopedQuery(
    ["customers", "list", { page: 1, pageSize: 100 }],
    () => customersService.list({ page: 1, pageSize: 100 }),
  );

  const tagSegments: Segment[] = useMemo(() => {
    const counts = new Map<string, number>();
    sample.data?.items.forEach((c) =>
      c.tags.forEach((t) => counts.set(t, (counts.get(t) ?? 0) + 1)),
    );
    const builtInTags = new Set(
      BUILT_IN.map((s) => s.filter.tag).filter(Boolean),
    );
    return [...counts.entries()]
      ...
      .map(([tag]) => ({
        key: `tag-${tag}`,
        name: tag,
        description: `Customers tagged ${tag}`,
        icon: Tag,
        filter: { tag },
      }));
  }, [sample.data]);
  ...
  return (
    <PageShell>
      <PageHeader
        title="Segments"
        description="Groups of customers you can open as a filtered list, based on status and tags."
      />
      <ModuleNav moduleKey="customers" />

      <Notice tone="neutral" icon={Info} className="mb-5">
        Segments are saved filters over customer tags and status. Add a tag to a
        customer (for example <span className="font-medium">vip</span>) and they
...
function SegmentCard({ segment }: { segment: Segment }) {
  const count = useScopedQuery(
    ["customers", "count", "segment", segment.filter],
    () => customersService.list({ ...segment.filter, pageSize: 1 }),
  );
      ...
      <div className="flex items-start justify-between gap-3">
        <span
          className={cn(
            ...
            segment.tone ?? "bg-surface-muted text-muted-foreground",
          )}
        >
        ...
        {count.isPending ? (
          ...
          <span
            ...
            aria-label={
              count.data ? `${count.data.total} customers` : "Count unavailable"
            }
          >
