...
export interface BillingService {
  ...
  invoices(
    params: ListParams & { customerId?: string; overdue?: boolean },
  ...
  payments(params: {
    ...
    Page<Payment & { invoice_number?: string; customer_name?: string | null }>
  >;
}
...
const live: BillingService = {
  ...
  invoice: (id) =>
    apiRequest("GET", `/billing/invoices/${id}`, invoiceDetailSchema),
  createInvoice: (input) =>
    apiRequest("POST", "/billing/invoices", invoiceDetailSchema, {
      body: input,
    }),
  invoiceAction: (id, action) =>
    apiRequest("POST", `/billing/invoices/${id}/actions`, invoiceDetailSchema, {
      body: { action },
    }),
  recordPayment: (id, input) =>
    apiRequest("POST", `/billing/invoices/${id}/payments`, paymentSchema, {
      body: input,
    }),
  payments: ({ page = 1, pageSize = 25 }) =>
    apiRequest("GET", "/billing/payments", pageSchema(paymentSchema), {
...
function view(invoice: InvoiceDetail): InvoiceDetail {
  const overdue =
    ...
    !!invoice.due_date &&
    invoice.due_date < dateOnly(0);
  const actions =
    invoice.status === "draft"
      ? ["issue", "void"]
      : invoice.status === "issued"
        ? toCents(invoice.amount_paid) > BigInt(0)
          ? ["record_payment"]
          : ["record_payment", "void"]
  ...
  return {
    ...invoice,
    is_overdue: overdue,
    balance_due: sumMoney([invoice.total, `-${invoice.amount_paid}`]),
...
const demo: BillingService = {
  async summary() {
    ...
    const collected = business.invoices
    ...
    const rows = demoBusiness()
      ...
      .filter(
        (i) =>
          ...
          (!search ||
            matches(i.number, search) ||
            matches(i.customer_name, search)),
      )
      .sort((a, b) => b.created_at.localeCompare(a.created_at));
    return paginate(
      rows.map(({ lines: _l, payments: _p, next_actions: _n, ...i }) => i),
      page,
      pageSize,
    );
  },
  ...
  async createInvoice(input) {
    ...
    const lines = input.lines.map((l, i) => {
      ...
      if (toCents(l.discount) > toCents(gross))
      ...
      return {
        ...
        discount: l.discount,
        line_total: sumMoney([gross, `-${l.discount}`]),
        gross,
      };
    });
  ...
  async recordPayment(invoiceId, input) {
    ...
    if (!["issued", "partially_paid"].includes(invoice.status))
      rule("INVOICE_NOT_OPEN", "Payments need an issued invoice");
    const balance = toCents(invoice.total) - toCents(invoice.amount_paid);
    const amount = toCents(input.amount);
    if (amount <= BigInt(0))
      rule("INVALID_AMOUNT", "Enter an amount greater than zero");
    if (amount > balance)
      rule("OVERPAYMENT", "Payment exceeds the balance due");
    const payment: Payment = {
      ...
      method: input.method,
      received_on: input.received_on || dateOnly(0),
    ...
    invoice.status =
      toCents(invoice.amount_paid) === toCents(invoice.total)
        ? "paid"
        : "partially_paid";
    return payment;
  },
