...
import { FormActions, FormField, FormSection, useUnsavedChangesWarning } from "@/components/app/forms";
...
const lineSchema = z
  .object({
    description: z.string().trim().min(1, "Describe what you're billing for").max(500, "Keep it under 500 characters"),
    quantity: z
      .string()
      .trim()
      .regex(QUANTITY_PATTERN, "Use a number like 1 or 2.5")
      .refine((v) => scaled(v, 3) > BigInt(0), "Must be more than zero"),
    unit_price: z.string().trim().regex(MONEY_PATTERN, "Use an amount like 120.00"),
    discount: z.string().trim().regex(MONEY_PATTERN, "Use an amount like 5.00"),
  })
  .superRefine((line, ctx) => {
    if (!MONEY_PATTERN.test(line.discount) || !MONEY_PATTERN.test(line.unit_price)) return;
    if (!QUANTITY_PATTERN.test(line.quantity)) return;
    if (scaled(line.discount, 2) > lineGrossCents(line.unit_price, line.quantity)) {
      ctx.addIssue({ code: "custom", path: ["discount"], message: "Discount can't exceed the line amount" });
    }
  });
...
const EMPTY_LINE = { description: "", quantity: "1", unit_price: "", discount: "0.00" };
...
function InvoiceForm() {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: { customer: null, due_date: "", notes: "", lines: [{ ...EMPTY_LINE }] },
    mode: "onTouched",
  });
  ...
  const onSubmit = handleSubmit((values) => {
    ...
    create.mutate(
      ...
      { onError: (e) => setServerError(errorMessage(e, "The invoice couldn't be created.")) },
    );
  });
  ...
  return (
    <PageShell width="default">
      ...
      />
      <Notice tone="info" icon={Info} title="This creates a draft" className="mb-5">
        Nothing is sent and no balance is owed until you issue it. Review the draft, then choose{" "}
        <span className="font-medium">Issue invoice</span> when it's ready.
      </Notice>
      ...
      <form onSubmit={onSubmit} noValidate>
        ...
        <div className="rounded-xl border border-border bg-surface p-4 shadow-sm sm:p-6">
          <FormSection title="Bill to" description="Invoices are issued to an existing, active customer.">
            <FormField label="Customer" htmlFor="invoice-customer" required error={errors.customer?.message}>
              <Controller
                control={control}
                name="customer"
          ...
          <FormSection title="Line items" description="Quantities allow up to 3 decimals; amounts up to 2.">
                ...
                return (
                  ...
                  >
                    <div className="col-span-3 sm:col-span-1">
                      <label htmlFor={`line-${index}-description`} className="mb-1 block text-xs font-medium sm:sr-only">
                        Description
                      </label>
                      <Input
                    ...
                    <div>
                      <label htmlFor={`line-${index}-quantity`} className="mb-1 block text-xs font-medium sm:sr-only">
                        Quantity
                      </label>
                      <Input
                    ...
                    <div>
                      <label htmlFor={`line-${index}-price`} className="mb-1 block text-xs font-medium sm:sr-only">
                        Unit price
                      </label>
                      <Input
                        id={`line-${index}-price`}
                        ...
                        placeholder="0.00"
                        className="tabular"
                        aria-invalid={Boolean(lineErrors?.unit_price) || undefined}
                        disabled={saving}
                    ...
                    <div>
                      <label htmlFor={`line-${index}-discount`} className="mb-1 block text-xs font-medium sm:sr-only">
                        Discount
                      </label>
                      <Input
                        ...
                        inputMode="decimal"
                        className="tabular"
                        aria-invalid={Boolean(lineErrors?.discount) || undefined}
                        disabled={saving}
            ...
            {errors.lines?.root?.message && <LineError message={errors.lines.root.message} />}
            <Button
              type="button"
              variant="secondary"
              size="sm"
          ...
          <FormSection title="Terms" description="Optional details printed on the invoice.">
            <FormField
              label="Due date"
              htmlFor="invoice-due"
              optional
            ...
            <FormField label="Notes" htmlFor="invoice-notes" optional error={errors.notes?.message}>
              <Textarea
                id="invoice-notes"
                rows={3}
...
function lineNetCents(line: { quantity?: string; unit_price?: string; discount?: string } | undefined) {
  if (!line) return BigInt(0);
  const quantity = (line.quantity ?? "").trim();
  const price = (line.unit_price ?? "").trim();
  const discount = (line.discount ?? "").trim();
  if (!QUANTITY_PATTERN.test(quantity) || !MONEY_PATTERN.test(price)) return BigInt(0);
...
function TotalsPreview({
  ...
  for (const line of lines) {
    const quantity = (line?.quantity ?? "").trim();
    const price = (line?.unit_price ?? "").trim();
    if (!QUANTITY_PATTERN.test(quantity) || !MONEY_PATTERN.test(price)) continue;
  ...
  return (
    <div className="ml-auto w-full max-w-sm rounded-lg bg-surface-muted/70 p-3.5 text-[13px]" aria-live="polite">
      {loading ? (
        ...
        <dl className="space-y-1.5">
          ...
          <div className="flex justify-between gap-4">
            ...
            <dd className="tabular">{discounts > BigInt(0) ? `−${money(discounts)}` : money(BigInt(0))}</dd>
          </div>
          <div className="flex justify-between gap-4">
            <dt className="text-muted-foreground">Tax{taxRate ? ` (${formatRate(taxRate)})` : ""}</dt>
            <dd className="tabular">{taxRate ? money(tax) : "Calculated on save"}</dd>
          </div>
      ...
      <p className="mt-2 text-xs text-muted-foreground">
        Preview only. Final totals are calculated by the server when the draft is saved.
      </p>
    </div>
  );
}
