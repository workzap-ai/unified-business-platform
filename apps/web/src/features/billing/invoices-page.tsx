...
const VIEWS: SavedView[] = [
  ...
  { id: "partially_paid", name: "Partially paid", params: { status: "partially_paid" }, builtIn: true },
  { id: "paid", name: "Paid", params: { status: "paid" }, builtIn: true },
  { id: "overdue", name: "Overdue", params: { overdue: "true" }, builtIn: true },
...
const COLUMNS: Column<Invoice>[] = [
  {
    ...
    header: "Number",
    cell: (i) => (
      <Link href={`/billing/invoices/${i.id}`} className="font-medium whitespace-nowrap hover:underline">
        {i.number}
      </Link>
    ),
  },
  {
    key: "customer",
    header: "Customer",
    cell: (i) => <span className="block max-w-56 truncate">{i.customer_name ?? "—"}</span>,
    hideBelow: "sm",
  },
  { key: "status", header: "Status", cell: (i) => <InvoiceStatus invoice={i} /> },
  {
    key: "issue_date",
    header: "Issued",
    cell: (i) =>
      i.issue_date ? (
        <span className="tabular whitespace-nowrap">{formatDay(i.issue_date)}</span>
      ) : (
  ...
  { key: "due_date", header: "Due", cell: (i) => <DueDate invoice={i} />, hideBelow: "md", optional: true },
  {
    key: "total",
    header: "Total",
    align: "right",
    cell: (i) => <span className="tabular whitespace-nowrap">{formatMoney(i.total, i.currency)}</span>,
    hideBelow: "lg",
    optional: true,
  },
  {
    ...
    cell: (i) =>
      i.status === "void" ? (
        ...
        <span className="tabular font-medium whitespace-nowrap">{formatMoney(i.balance_due, i.currency)}</span>
      ),
  },
];
...
function InvoicesList() {
  ...
  const [state, setState, reset] = useUrlState({ search: "", status: "", overdue: "", page: "1" });
  ...
  const query = useScopedQuery(["invoices", params], () => billingService.invoices(params), {
    placeholderData: (previous) => previous,
  });
  const { hidden, toggle } = useColumnVisibility("billing-invoices", COLUMNS);
  const activeCount = [state.search, state.status, state.overdue].filter(Boolean).length;
  ...
  return (
    <PageShell>
      ...
      <SavedViews
        ...
        views={VIEWS}
        current={{ search: state.search, status: state.status, overdue: state.overdue }}
      ...
      {query.data && (
        <Pagination
          ...
          onPage={(next) => setState({ page: String(next) }, { resetPage: false })}
        />
      )}
    </PageShell>
  );
