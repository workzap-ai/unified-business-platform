...
function InventoryContent() {
  const summary = useScopedQuery(["reports", "inventory"], () =>
    reportsService.inventory(),
  );
  const levels = useScopedQuery(
    ["inventory", "levels", { lowOnly: true, pageSize: 25 }],
  ...
  const columns: Column<StockLevel>[] = [
    ...
    },
    {
      ...
      align: "right",
      hideBelow: "lg",
      cell: (r) => <span className="tabular">{formatNumber(r.reserved)}</span>,
    },
    {
      key: "available",
      header: "Available",
    ...
    },
    {
      key: "status",
      header: "Status",
      cell: (r) => <StatusBadge status={r.available <= 0 ? "out" : "low"} />,
    },
  ];
    ...
    <>
      <MetricGrid className="xl:grid-cols-3">
        <MetricCard
          ...
          detail="On-hand quantity × current price"
        />
        <MetricCard
        ...
        <MetricCard
          ...
          value={
            levels.isError
              ? "—"
              : formatNumber(rows?.filter((r) => r.available <= 0).length ?? 0)
          }
      ...
      <SectionHeader
        ...
        description={
          levels.data && levels.data.total > 25
            ? `Showing 25 of ${formatNumber(levels.data.total)} stock rows.`
            : "Stock rows at or below their threshold, per location."
        }
        actions={
          <>
            <ExportMenu
      ...
      <DataTable
        ...
        empty={
          <EmptyState
            ...
            description="No item is at or below its low-stock threshold."
          />
        }
      />
      <Link
        href={LOW_HREF}
        className="mt-3 inline-flex items-center gap-1 text-[13px] font-medium text-primary hover:underline sm:hidden"
      >
        Open in stock <ArrowRight className="size-3.5" />
      </Link>
    </>
