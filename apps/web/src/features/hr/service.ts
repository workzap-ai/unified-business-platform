...
export interface HRService {
  ...
  employees(
    params: ListParams & { departmentId?: string },
...
const live: HRService = {
  ...
  employees: ({ page = 1, pageSize = 25, search, status, departmentId }) =>
    apiRequest("GET", "/hr/employees", pageSchema(employeeSchema), {
      query: {
        page,
        page_size: pageSize,
        search,
  ...
  create: (input) =>
    apiRequest("POST", "/hr/employees", employeeSchema, { body: input }),
  update: (id, input) =>
    apiRequest("PATCH", `/hr/employees/${id}`, employeeSchema, { body: input }),
};
...
const demo: HRService = {
  async headcount() {
    ...
    for (const e of employees.filter((e) => e.status !== "terminated"))
      byDept.set(
        e.department_name ?? "Unassigned",
        (byDept.get(e.department_name ?? "Unassigned") ?? 0) + 1,
      );
    return {
  ...
  async create(input) {
    ...
    if (
      (input.salary || input.salary_currency) &&
      !demoSession()?.permissions.includes("hr.sensitive")
    )
      throw new ApiError(
        422,
        "SENSITIVE_FIELD",
        undefined,
        "Compensation needs HR sensitive access",
      );
  ...
  async update(id, input) {
    ...
    if (
      ("salary" in input || "salary_currency" in input) &&
      !demoSession()?.permissions.includes("hr.sensitive")
    )
      throw new ApiError(
        422,
    ...
    if (input.department_id !== undefined)
      employee.department_name =
        demoBusiness().departments.find((d) => d.id === input.department_id)
          ?.name ?? null;
    if (employee.status === "terminated" && !employee.termination_date)
      employee.termination_date = new Date().toISOString().slice(0, 10);
    return redact(employee);
  },
};
