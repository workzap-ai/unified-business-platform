...
const leadFormSchema = z.object({
  title: z
    .string()
    .trim()
    .min(1, "Give the lead a short title")
...
function toForm(
  ...
  return {
    ...
    customer: lead.customer_id
      ? { id: lead.customer_id, name: lead.customer_name ?? "Customer" }
      : null,
...
export function LeadFormDialog({
  ...
  const save = useScopedMutation(
    (values: LeadFormValues) => {
      ...
      return lead
        ? salesService.update(lead.id, input)
    ...
    try {
      ...
      if (error instanceof ApiError) {
        for (const [field, message] of Object.entries(error.fields)) {
          ...
          if (key in leadFormSchema.shape)
            form.setError(key as keyof LeadFormValues, {
              type: "server",
              message,
            });
        }
      ...
      <DialogContent size="md">
        ...
        >
          <DialogHeader
            ...
            description={
              editing
                ? "Update the lead's details. Use the stage buttons to move it through the pipeline."
                : "Track a potential sale from first contact to won or lost."
            }
          />
          <DialogBody className="space-y-4">
            ...
            <FormField
              ...
              help="Link an existing customer to create quotes from this lead."
            >
              <Controller
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <FormField
                label={
                  currency ? `Estimated value (${currency})` : "Estimated value"
                }
                htmlFor="lead-value"
                optional
              ...
              <FormField
                ...
                help={
                  lead?.source === "pi"
                    ? "Captured by PI; the source can't be changed."
                    : undefined
                }
              >
                <NativeSelect
                  id="lead-source"
                  disabled={lead?.source === "pi"}
                  {...form.register("source")}
                >
                  {lead?.source === "pi" && (
                    <option value="pi">{LEAD_SOURCE_LABELS.pi}</option>
                  )}
            ...
            >
              <Textarea
                id="lead-notes"
                rows={4}
                placeholder="Context, budget, timing, decision makers…"
                {...form.register("notes")}
              />
            </FormField>
          </DialogBody>
          <DialogFooter>
            ...
            <Button
              type="submit"
              loading={saving}
              disabled={saving || (editing && !form.formState.isDirty)}
            >
