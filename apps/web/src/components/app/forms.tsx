...
export function FormSection({
  ...
  return (
    <section
      className={cn(
        "grid gap-x-8 gap-y-4 border-b border-border py-6 first:pt-0 last:border-0 lg:grid-cols-[240px_1fr]",
        className,
      )}
    >
      <div>
...
export function FormActions({
  ...
  return (
    ...
    >
      <span
        className="mr-auto flex items-center gap-1.5 text-[13px]"
        aria-live="polite"
      >
        {dirty ? (
          <>
...
/** Lives inside the dialog content, which unmounts on close, so typed text resets. */
      ...
      {(consequences?.length || requireText) && (
        <DialogBody className="space-y-3">
          {consequences && consequences.length > 0 && (
            <ul
              className={cn(
                "space-y-1.5 rounded-lg p-3 text-[13px]",
                destructive ? "bg-danger-soft" : "bg-surface-muted",
              )}
            >
              {consequences.map((c) => (
                <li key={c} className="flex gap-2">
                  <AlertTriangle
                    className={cn(
                      ...
                      destructive ? "text-danger" : "text-warning",
                    )}
                    aria-hidden="true"
                  />
...
export function Stepper({
  ...
  return (
    <ol
      className="scrollbar-thin mb-6 flex items-center gap-2 overflow-x-auto"
      aria-label="Progress"
    >
