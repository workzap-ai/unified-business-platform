      ...
      {Icon && (
        <div
          className={cn(
            ...
            tone === "pi" ? "bg-pi-soft text-pi" : "bg-surface-muted text-muted-foreground",
          )}
        >
          <Icon className={compact ? "size-4.5" : "size-5"} aria-hidden="true" />
        </div>
      )}
      <h3 className={cn("font-semibold", compact ? "mt-3 text-sm" : "mt-4 text-[15px]")}>{title}</h3>
      {description && (
  ...
  return (
    ...
    >
      ...
      <h3 className="mt-3.5 text-[15px] font-semibold">
        {title ?? (denied ? "Access restricted" : missing ? "Not found" : "Couldn't load this")}
      </h3>
      <p className="mt-1 max-w-sm text-[13px] text-muted-foreground">
        {denied
          ...
          : missing
            ...
            : errorMessage(error, "Something went wrong while loading. Please try again.")}
      </p>
...
export function InlineError({ message }: { message: string }) {
  return (
    <p role="alert" className="flex items-center gap-1.5 rounded-md bg-danger-soft px-3 py-2 text-[13px] text-danger">
...
export function Notice({
  ...
  return (
    <div className={cn("flex items-start gap-3 rounded-lg border px-3.5 py-3", tones[tone], className)}>
      {Icon && <Icon className="mt-0.5 size-4 shrink-0" aria-hidden="true" />}
      <div className="min-w-0 flex-1 text-[13px]">
        {title && <p className="font-semibold">{title}</p>}
        {children && <div className={cn("text-foreground-secondary", title && "mt-0.5")}>{children}</div>}
      </div>
