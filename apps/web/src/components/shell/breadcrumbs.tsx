...
const Context = createContext<{ record: Crumb[]; setRecord: (c: Crumb[]) => void }>({
  record: [],
  setRecord: () => {},
});
...
export function BreadcrumbProvider({ children }: { children: React.ReactNode }) {
  ...
  return <Context.Provider value={{ record, setRecord }}>{children}</Context.Provider>;
}
 ...
 */
export function useBreadcrumbs(crumbs: Crumb[], recent?: { href: string; kind: string }) {
  ...
  useEffect(() => {
    ...
    if (recent && last) pushRecent(scopeKey.join(":"), { href: recent.href, kind: recent.kind, title: last.label });
    return () => setRecord([]);
...
export function Breadcrumbs() {
  ...
  const trail: Crumb[] = [{ label: active.item.label, href: active.item.route }];
    ...
    <nav aria-label="Breadcrumb" className="min-w-0">
      <ol className="flex min-w-0 items-center gap-1 text-[13px]">
        {trail.map((crumb, index) => {
          ...
          return (
            <li key={`${crumb.label}-${index}`} className={last ? "min-w-0 truncate" : "hidden shrink-0 items-center gap-1 sm:flex"}>
              {last || !crumb.href ? (
