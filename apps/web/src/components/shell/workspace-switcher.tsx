    ...
    <Popover open={open} onOpenChange={setOpen}>
            ...
            <>
              <span className="min-w-0 flex-1">
                ...
                <span className="block truncate text-2xs text-sidebar-muted">
                  ...
                  {session?.roles[0]
                    ? session.roles[0].replace(/^\w/, (c) => c.toUpperCase())
                    : "Member"}
                </span>
              </span>
              <ChevronsUpDown
      ...
      >
        ...
        <ul className="space-y-px">
          ...
          {tenants.isError && (
            <li className="px-2 py-2 text-xs text-danger">
        ...
        <div className="mt-1 border-t border-border pt-1">
          ...
          <Link
            ...
            className="flex items-center gap-2 rounded-md px-2 py-1.5 text-[13px] text-muted-foreground hover:bg-surface-muted hover:text-foreground"
          >
            <Plus className="size-4" aria-hidden="true" /> Create a new
            workspace
          </Link>
        </div>
      </PopoverContent>
...
export function EnvironmentSwitcher() {
  ...
  const [open, setOpen] = useState(false);
  const current = environments.data?.find(
    (e) => e.id === session?.environment?.id,
  );
  const kind = (current?.kind ??
    session?.environment?.kind ??
    "production") as Environment["kind"];

  async function choose(env: Environment) {
    if (!session?.tenant || env.id === session.environment?.id)
          ...
          .map((env) => (
            ...
            >
              <span
                className={cn(
                  "size-2 rounded-full",
                  ENV_TONE[env.kind].split(" ")[1]?.replace("text-", "bg-"),
                )}
              />
              <span className="flex-1">
                <span className="block font-medium">{env.name}</span>
                <span className="block text-2xs text-muted-foreground capitalize">
                  {env.kind}
                </span>
              </span>
              {env.id === session.environment?.id && (
                <Check className="size-4 text-primary" />
              )}
            </button>
          ))}
        <p className="mt-1 border-t border-border px-2 pt-2 pb-1 text-2xs leading-4 text-muted-foreground">
          Data, PI configuration and WhatsApp numbers are separate per
          environment.
        </p>
      </PopoverContent>
    </Popover>
