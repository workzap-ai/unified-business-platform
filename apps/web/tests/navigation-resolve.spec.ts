...
import {
  ...
  validateCustomOrder,
  type NavContext,
} from "../src/features/navigation/resolve";
...
function ctx(
  ...
  products: Record<string, string[]> = { pi: ["text", "knowledge", "handoff"] },
): NavContext {
  return {
...
const mainLabels = (nav: ReturnType<typeof resolveNavigation>) =>
  nav.sections.find((s) => s.key === "main")!.items.map((i) => i.label);
...
test("PI requires installation and permission; no gaps when hidden", () => {
  expect(mainLabels(resolveNavigation(definitions, ctx(owner, {})))).toEqual(
    DEFAULT.filter((l) => l !== "PI"),
  );
  expect(
    mainLabels(
      resolveNavigation(definitions, ctx(owner.filter((p) => p !== "pi.read"))),
    ),
  ).not.toContain("PI");
});
...
test("custom order applies and a newly registered module appears by sort order", () => {
  ...
  const withFuture: NavDefinition[] = [
    ...definitions,
    {
      ...definitions.find((d) => d.key === "reports")!,
      key: "subscriptions",
      label: "Subscriptions",
      route: "/subscriptions",
      sort_order: 130,
  ...
  const denied = resolveNavigation(
    withFuture,
    ctx(owner.filter((p) => p !== "billing.read")),
    { main: custom },
  );
  expect(denied.sections[0]!.items.map((i) => i.key)).not.toContain(
    "subscriptions",
  );
});
...
test("merge inserts new items after their default neighbour and skips stale keys", () => {
  ...
  expect(mergeOrder(items, ["c", "b", "a"]).map((i) => i.key)).toEqual([
    "c",
    "b",
    "new",
    "a",
  ]);
  expect(mergeOrder(items, ["removed", "a"]).map((i) => i.key)).toEqual([
    "a",
    "b",
    "new",
    "c",
