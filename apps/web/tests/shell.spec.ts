...
test("unauthenticated visitors are sent to sign-in with a safe return path", async ({ page }) => {
...
test("sidebar renders the default order with PI as a top-level product", async ({ page }) => {
  await signIn(page);
  expect(await mainLabels(page)).toEqual(DEFAULT_MAIN);
  await expect(page.getByRole("navigation", { name: "Administration" }).getByRole("link", { name: "Audit Logs" })).toBeVisible();
  await expect(page.getByText("Sample data", { exact: true })).toBeVisible();
});

test("sidebar order can be changed by keyboard, persists across reload, and resets", async ({ page }) => {
  await signIn(page);
  ...
  const expected = ["Overview", "PI", "Customers / CRM", "Catalog", ...DEFAULT_MAIN.slice(4)];
...
test("permissions shape navigation (viewing as Support)", async ({ page }) => {
  ...
  await page.getByText("View as role").hover();
  await page.getByRole("menuitem", { name: "Support" }).click();
  await expect.poll(() => mainLabels(page)).toEqual(["Overview", "Customers / CRM", "Catalog", "PI", "Inventory", "Orders"]);
  await page.goto("/finance");
  await expect(page.getByRole("heading", { name: /don't have access/ })).toBeVisible();
});

test("PI disappears in an environment where the product is not enabled", async ({ page }) => {
  await signIn(page);
  ...
  await expect.poll(() => mainLabels(page)).toEqual(DEFAULT_MAIN.filter((l) => l !== "PI"));
});

test("mobile uses a drawer with the same registry-driven navigation", async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await signIn(page);
  await page.getByRole("button", { name: "Open navigation" }).click();
  expect(await mainLabels(page)).toEqual(DEFAULT_MAIN);
  await primaryNav(page).getByRole("link", { name: /Quotes/ }).click();
...
test("switching workspace shows that workspace's own data", async ({ page }) => {
  await signIn(page);
  await page.getByRole("button", { name: /Workspace: Northwind/ }).click();
  await page.getByRole("button", { name: "Brightline Studio" }).click();
  await expect(page.getByRole("button", { name: /Workspace: Brightline Studio/ })).toBeVisible();
